export async function searchDriver(drivers, order_info, io, redisAsyncClient, redisPubClient, db): Promise<boolean> {
    let options = {
    ...
    for (let i = 0; i < drivers_list.length; i++) {
        ...
        if (!isNaN(driver_id)) {
            driver_last_update = await redisAsyncClient.get(`driver_${driver_id}_location_last_update`);
            let driver_exists_in_order = await redisAsyncClient.get(`order_drivers_${order_info.id}`);
            if (driver_last_update !== null && driver_last_update >= Math.floor(new Date().getTime() / 1000) - 6000) {
                ...
                driver_online = await db.executeQuery(`SELECT id FROM driver_statuses WHERE user_id=${driver_id} AND driver_status='online'`);
                if(driver_online.length == 0){
                    continue
                }
                driver_tariff = await db.executeQuery(`SELECT id FROM driver_enabled_tariffs WHERE user_id=${driver_id} AND is_active=true AND tariff_id=${order_info.tariff_id}`);
    ...
    if (driver_id_to_send !== 0) {
        ...
        let all_order_drivers = await redisAsyncClient.get(`order_drivers_${order_info.id}`);
        ...
        await redisPubClient.set(`order_drivers_${order_info.id}`, JSON.stringify(all_order_drivers_arr), 'EX', 5 * 60);
        console.log(driver_id_to_send)
        if (socket_id == null) {
            return await searchDriver(drivers, order_info, io, redisAsyncClient, redisPubClient, db)
        } else {
            await redisPubClient.set(`order_driver_${order_info.id}`, driver_id_to_send, 'EX', 60 * 60);
