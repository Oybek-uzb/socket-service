...
export class ServerGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleDriverLocations(socket: Socket, msg, callback) {
    let options = {
    ...
    this.drivers.nearby({ latitude: msg.latitude, longitude: msg.longitude }, msg.distance, options, function (err, driver) {
      if (err) {
        console.error(err)
      }
      else {
  ...
  handleSendMessage(socket: Socket, msg) {
    this.redisPubClient.get(`sid${msg.to}${msg[msg.to + '_id']}`, this.emitToMessages.bind(this, msg));
  }
  ...
  async emitToMessages(msg, err, value) {
    if (err) {
      ...
      await this.db.executeQuery(`INSERT INTO chat_messages (user_type,driver_id,client_id,ride_id,order_id,message_type,content,created_at) VALUES ('${msg.from}',${msg.driver_id},${msg.client_id},${msg.ride_id},${msg.order_id},'${msg.type}','${msg.content}','${moment().subtract(5, 'hours').format('YYYY-MM-DD HH:mm:ss')}')`);
    }
  }

  @SubscribeMessage('set_client_location')
  handleSetClientLocation(socket: Socket, msg) {
    this.redisPubClient.get(`siddriver${msg.driver_id}`, this.emitToClientLocation.bind(this, msg));
  }
  ...
  handleSetDriverOffline(socket: Socket, msg) {
    this.drivers.removeLocation('driver_' + msg.driver_id, function (err, reply) {
  ...
  handleSetDriverLocation(socket: Socket, msg) {
    this.drivers.addLocation('driver_' + msg.driver_id, { latitude: msg.latitude, longitude: msg.longitude }, this.addLocation.bind(this, msg))
  }
  ...
  addLocation(msg, redis_err, reply) {
    if (redis_err) {
      ...
      process.env.TZ = "Asia/Tashkent";
      this.redisPubClient.set(`driver_${msg.driver_id}_location_last_update`, Math.floor(new Date().getTime() / 1000), 'EX', 30 * 60, function (err, value) {
        if (err) console.error(err);
        // console.log(value, msg.driver_id)
      });
      if (msg.client_id !== 0) {
        this.redisPubClient.get(`sidclient${msg.client_id}`, this.emitToDriverLocation.bind(this, msg));
      }
    }
  }
